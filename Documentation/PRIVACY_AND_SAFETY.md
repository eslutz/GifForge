...
The containing app discloses that prompts and selected images may be sent through the GifForge backend to external AI media providers. It also discloses that prompt planning and caption suggestions use local Apple models where available.
...
The app keeps local GIF history only while useful to the user. After validation, moderation, and provider submission, the backend stores minimized job state: caption mode without visible caption text, no raw `originalPrompt`, source-media metadata without source-media bytes, and source-image dimensions/context without the processed JPEG bytes. Source media needed for retry stays on the client in the active-generation snapshot, and the app asks the user before resubmitting it. The backend stores an expiration timestamp with each generation job and returns `410 Gone` after that point instead of exposing status or result links. Deployed defaults expire job rows after 24 hours and delete temporary provider result blobs after 2 days through Azure Storage lifecycle policy. A future authenticated deletion endpoint is still appropriate if the product adds user accounts or user-linked backend history.
