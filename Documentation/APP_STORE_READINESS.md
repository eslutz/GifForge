...
- `scripts/export-app-store-submission-package.rb` assembles the public metadata, App Review notes, privacy policy, and screenshot manifests into an ignored App Store Connect handoff package; final `--require-screenshots` mode still requires physical Messages screenshots.
...
- The client workflow runs focused validators for signing, metadata, App Store submission package generation, and physical-device evidence tooling instead of a broad release-readiness script.
- `scripts/smoke-backend.sh` checks `/health` and verifies protected generation routes reject unauthenticated requests when App Attest is required.
- The manual `Deploy Nonprod` workflow deploys an immutable GHCR backend image tag to the existing `rg-gifforge-nonprod` resource group and runs the backend smoke test against the resulting Container Apps URL.
- The manual `Deploy Prod` workflow deploys an immutable GHCR backend image tag to `rg-gifforge-prod` through the `prod` GitHub environment, requires production App Attest configuration, disables the demo bypass, and health-checks `/health`.
...
- Nonprod was redeployed with image `ghcr.io/eslutz/gifforge-backend:14cc2952c2311bb78532ca389b09566e51cb4579` using Azure deployment `gifforge-nonprod-scalezero-202606150620`.
- Resource group: `rg-gifforge-nonprod`.
- API Container App: `gifforge-nonprod-mamh4mnpf-api`.
- Worker Container App: `gifforge-nonprod-mamh4mnpf-worker`.
- Nonprod URL: `https://gifforge-nonprod-mamh4mnpf-api.greencliff-56b7d6e3.eastus.azurecontainerapps.io`.
- API Container App reports image `ghcr.io/eslutz/gifforge-backend:14cc2952c2311bb78532ca389b09566e51cb4579`, `minReplicas=0`, `maxReplicas=5`, and HTTP scale rule `concurrentRequests=50`.
- Worker Container App reports image `ghcr.io/eslutz/gifforge-backend:14cc2952c2311bb78532ca389b09566e51cb4579`, `minReplicas=0`, `maxReplicas=5`, and Azure Queue scale rule `generation-jobs` with queue length `1` and user-assigned managed identity.
...
- On June 15, 2026, read-only deployment setup checks confirmed the active Azure subscription `fba65efe-a59e-4177-a27a-afc3ee0b2172`, tenant `6131bdcf-4c9a-4d55-ac15-78135afd4637`, the existing `rg-gifforge-nonprod` resource group in `eastus`, and the existing GitHub `nonprod` environment.
- On June 15, 2026, `scripts/setup-azure-oidc.sh --environment nonprod --subscription-id fba65efe-a59e-4177-a27a-afc3ee0b2172 --tenant-id 6131bdcf-4c9a-4d55-ac15-78135afd4637` produced the expected dry run: create or reuse `GifForge-GitHub-Actions-nonprod`, add federated subject `repo:eslutz/GifForge:environment:nonprod`, grant `Contributor` and `Role Based Access Control Administrator` only at `/subscriptions/fba65efe-a59e-4177-a27a-afc3ee0b2172/resourceGroups/rg-gifforge-nonprod`, and set `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID` on the GitHub `nonprod` environment.
...
- `Deploy Prod` workflow proof remains pending until `rg-gifforge-prod`, the `prod` GitHub OIDC identity, production App Attest values, external provider settings, and an immutable image tag are available.
- On June 15, 2026, `az group show --name rg-gifforge-prod` returned `ResourceGroupNotFound`; production resource-group bootstrap remains pending.
...
- On June 15, 2026, `az deployment sub what-if --name gifforge-prod-bootstrap-whatif-20260615-continuation --location eastus --template-file infra/main.subscription.bicep ... minReplicas=0 workerMinReplicas=0 maxReplicas=10` completed with no deployment errors.
...
- Open GifForge from the Messages app drawer in compact mode.
...
- GifForge inserts GIFs into the Messages compose field using attachment insertion only; it never auto-sends messages.
...
- Select the first real AI media provider, run text-to-GIF and image-to-GIF external-provider preflights, validate the provider onboarding evidence, and confirm production `/health` reports `mode=external`.
