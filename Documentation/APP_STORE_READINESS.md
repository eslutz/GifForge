...
- The client workflow runs `scripts/verify-release-readiness.rb` to check the iOS 26.5 target, v1 no-sticker/no-Image-Playground source-code invariants, iMessage extension metadata, local caption re-render wiring, backend expiration propagation, known App Store metadata placeholders, and tracked app/iMessage icon catalog completeness.
