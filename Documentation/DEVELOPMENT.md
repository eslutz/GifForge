...
The `Deploy Nonprod` GitHub Actions workflow can deploy and smoke-test `rg-gifster-nonprod` manually. It uses resource-group-scope deployment against the existing nonprod resource group. Configure Azure OIDC secrets `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID`, add a federated credential with subject `repo:eslutz/Gifster:environment:nonprod`, and grant the deployment identity `Contributor` plus `Role Based Access Control Administrator` at the `rg-gifster-nonprod` scope. Dispatch the workflow with the backend GHCR image tag to deploy. Use its demo App Attest bypass input only for controlled nonprod smoke tests.
