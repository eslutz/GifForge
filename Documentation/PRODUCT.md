...
GifForge is a GIPHY-style iMessage app for making custom animated GIFs instead of searching a public content library. The primary surface is the Messages extension. The containing app explains the product, exposes privacy and development settings, and shows local generation history.

## Core Flows

### Text-to-GIF

1. User opens GifForge from the Messages app drawer.
2. User enters a prompt.
3. The app plans the request locally with Apple Foundation Models where available.
4. The app submits a structured request to the GifForge backend.
...
1. User opens GifForge from Messages.
