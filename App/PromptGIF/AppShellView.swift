...
struct AppShellView: View {
  ...
  var body: some View {
    TabView {
      ...
      .tabItem {
        Label("PromptGIF", systemImage: "sparkles")
      }
    ...
    .alert("PromptGIF", isPresented: Binding(
...
private struct OverviewView: View {
  var body: some View {
    List {
      ...
      Section("Privacy") {
        Text("Prompts and selected images are sent through your PromptGIF backend for media generation. Prompt cleanup and caption suggestions run locally when Apple Foundation Models are available.")
        Text("Generated GIFs are stored locally for recent history and can be cleared from this app.")
      }

      Section("Messages") {
        Text("Open PromptGIF from the Messages app drawer, generate a GIF, preview it, then insert it into the compose field. Messages always requires you to send manually.")
      }
    }
    .navigationTitle("PromptGIF")
  }
}
