...
struct ActiveGenerationStoreTests {
  ...
  func backendExpiredActiveGenerationIsCleared() async throws {
    ...
    try await store.save(snapshot)
    let restored = try await store.load(now: try #require(GifForgeISO8601DateParser.date(from: "2026-06-15T12:00:01Z")))
    let secondLoad = try await store.load(now: try #require(GifForgeISO8601DateParser.date(from: "2026-06-15T12:00:02Z")))
  ...
  private func temporaryDirectory() throws -> URL {
    let url = FileManager.default.temporaryDirectory
      .appending(path: "GifForgeCoreTests-\(UUID().uuidString)", directoryHint: .isDirectory)
