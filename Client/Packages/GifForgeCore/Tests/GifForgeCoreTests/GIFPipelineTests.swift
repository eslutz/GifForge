...
struct GIFPipelineTests {
  ...
  func rendersGIFLocally() throws {
    ...
    let outputURL = FileManager.default.temporaryDirectory
      .appending(path: "gifforge-test-\(UUID().uuidString).gif")
  ...
  func extractsFramesFromMP4() async throws {
    ...
    let outputURL = FileManager.default.temporaryDirectory
      .appending(path: "gifforge-mp4-test-\(UUID().uuidString).gif")
  ...
  func gifRendererDownsamplesForMessagesLimits() throws {
    ...
    let outputURL = FileManager.default.temporaryDirectory
      .appending(path: "gifforge-limits-test-\(UUID().uuidString).gif")
  ...
  func gifRendererPreservesDurationWhenSamplingFrames() throws {
    ...
    let outputURL = FileManager.default.temporaryDirectory
      .appending(path: "gifforge-duration-test-\(UUID().uuidString).gif")
  ...
  private func makeTestMovie() throws -> URL {
    let url = FileManager.default.temporaryDirectory
      .appending(path: "gifforge-test-source-\(UUID().uuidString).mp4")
  ...
  private func pixelBuffer(width: Int, height: Int, frameIndex: Int) throws -> CVPixelBuffer {
    ...
    guard status == kCVReturnSuccess, let buffer = maybeBuffer else {
      throw GifForgeError.mediaRenderingFailed(message: "Could not create test pixel buffer.")
    }
    ...
    guard let baseAddress = CVPixelBufferGetBaseAddress(buffer) else {
      throw GifForgeError.mediaRenderingFailed(message: "Could not address test pixel buffer.")
    }
  ...
  private func testImage(red: CGFloat) throws -> CGImage {
    guard
      ...
      throw GifForgeError.mediaRenderingFailed(message: "Could not create test image.")
    }
    ...
    guard let image = context.makeImage() else {
      throw GifForgeError.mediaRenderingFailed(message: "Could not finish test image.")
    }
