...
public struct GIFRenderer {
  ...
  public func render(
    ...
    guard !frames.isEmpty else {
      throw GifForgeError.mediaRenderingFailed(message: "Cannot render a GIF without frames.")
    }
    ...
    guard let destination = CGImageDestinationCreateWithURL(
      ...
      throw GifForgeError.mediaRenderingFailed(message: "Could not create GIF destination.")
    }
    ...
    guard CGImageDestinationFinalize(destination) else {
      throw GifForgeError.mediaRenderingFailed(message: "ImageIO could not finalize the GIF.")
    }
    ...
    if let maxFileSizeBytes = options.maxFileSizeBytes {
      ...
      guard fileSize <= maxFileSizeBytes else {
        throw GifForgeError.mediaRenderingFailed(message: "The GIF is too large for Messages. Try a shorter or simpler prompt.")
      }
    }
  ...
  private func downsample(_ image: CGImage, maxPixelDimension: Int) throws -> CGImage {
    ...
    guard
      ...
      throw GifForgeError.mediaRenderingFailed(message: "Could not downsample GIF frames.")
    }
    ...
    guard let result = context.makeImage() else {
      throw GifForgeError.mediaRenderingFailed(message: "Could not create downsampled GIF frame.")
    }
