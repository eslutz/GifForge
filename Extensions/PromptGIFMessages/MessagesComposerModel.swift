...
final class MessagesComposerModel {
  ...
  @ObservationIgnored private let activeJobKey = "activePromptGIFJob"
  ...
  private func finish(
    ...
    guard let downloadURL = completed.downloadURL else {
      throw PromptGIFError.jobFailed(message: "Backend completed without a result URL.")
    }
    ...
    let outputURL = outputDirectory.appending(path: "PromptGIF-\(UUID().uuidString).gif")
  ...
  private func backendClient() -> PromptGIFBackendClient {
    let rawValue = defaults.string(forKey: "backendBaseURL") ?? "http://127.0.0.1:8787"
    return PromptGIFBackendClient(baseURL: URL(string: rawValue) ?? URL(string: "http://127.0.0.1:8787")!)
  }
